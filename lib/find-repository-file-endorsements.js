 ...
 * @returns {Promise<{endorsements: import("..").DatabaseColumns[], lastCommitSha: string} | undefined>}
 */
export async function findRepositoryFileEndorsements(
  ...
  const ownerId = searchResult.repository.owner.id;
  ...
  return { endorsements: newEndorsements, lastCommitSha: commits[0].sha };
}
